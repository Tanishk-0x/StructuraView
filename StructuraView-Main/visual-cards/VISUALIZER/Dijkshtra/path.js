...
let startingPointX = 7 ; 
let startingPointY = 7;
let endingPointX = 7;
let endingPointY = 27;

let out = document.getElementById('ints') ; 

// speed of algorithm 
let speed = 10 ; 

// visualization start he ya stoped he .. 
...
class Deque {
  ...
  // table ki m adad sse grid jaisa structue create kr lenge .. 
const tab = () => {
    const allowDrop = (e) => {
      e.preventDefault();
    };
...
function sleep(ms) {
  ...
  const Dijkstra = async () => {
    ...
    if (vis[endingPointX][endingPointY]) {
      ...
      alert('No possible path between source to destination');
    }
    currentlyRunning = false;
  };
  ...
  // function grid ko clear krne ke liye .. 
  const clearGrid = () => {
  ...
  let bt1 = document.getElementById('visualizeButton') ; 



  let bt2 = document.getElementById('clearButton') ; 

  bt2.addEventListener('click', () => {

  }); 

  let what = document.getElementById('hsh') ; 
